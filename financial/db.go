...
var (
	...
	createOPdatabook = `
		CREATE TABLE IF NOT EXISTS operational_databook (
		...
		ORDER BY (code, table, category, quarter, name)
	`
	...
	createfinanceResults = `
		...
		ORDER BY (code, category, month, name)
	`
